...
async function run() {
  ...
  const statements = [
    ...
    "CREATE TABLE IF NOT EXISTS `invoices` (`id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT, `company_id` BIGINT UNSIGNED NOT NULL, `client_id` BIGINT UNSIGNED NOT NULL, `amount` DECIMAL(10,2) NOT NULL, `due_date` DATE NOT NULL, `status` VARCHAR(30) NOT NULL DEFAULT 'pending', `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY (`id`), KEY `idx_invoices_company_id` (`company_id`), KEY `idx_invoices_client_id` (`client_id`), KEY `idx_invoices_due_date` (`due_date`), KEY `idx_invoices_status` (`status`), CONSTRAINT `fk_invoices_company` FOREIGN KEY (`company_id`) REFERENCES `companies` (`id`) ON UPDATE CASCADE ON DELETE CASCADE, CONSTRAINT `fk_invoices_client` FOREIGN KEY (`client_id`) REFERENCES `clients` (`id`) ON UPDATE CASCADE ON DELETE RESTRICT) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci",
